from new.model import PortalSnapshot, PortalSnapshotQuality
...
#    pprint.pprint(D.raw)
    ...
    for i in db.Session.query(PortalSnapshot,PortalSnapshotQuality).filter(PortalSnapshot.snapshot==1625).outerjoin(PortalSnapshotQuality, PortalSnapshot.portalid==PortalSnapshotQuality.portalid).add.all():
        print row2dict(i)
