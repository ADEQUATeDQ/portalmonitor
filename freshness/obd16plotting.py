...
def plot_tf_values(samples, values, labels, colors, estimators, filename):
...
def comparison():
    with open('any_i10_d20_p0.8.json') as f:
        res = json.load(f)
    ...
    plot_tf_values(samples=xlabels, values=avg_p, estimators=estimators, labels=ylabels, colors=colors, filename='tmp/tf_values.pdf')


def age():
    with open('age_regular.json') as f:
        res = json.load(f)
    print 'total articles', len(res)
